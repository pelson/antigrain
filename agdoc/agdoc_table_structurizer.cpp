...
namespace agdoc
{
    ...
    //------------------------------------------------------------------
    void table_content_extractor::content(const element& e, const char_type* c, unsigned len)
    {
        if(m_skip_element == 0)
        {
            m_table.add(c, len);
...
//        if(m_cfg.keyword_exists(keyword_formatless_elements, e.name(), e.name_len()))
    ...
    //------------------------------------------------------------------
    void table_structurizer::content(const element& e, const char_type* str, unsigned len)
    {
        if(m_skip_element == 0)
        {
